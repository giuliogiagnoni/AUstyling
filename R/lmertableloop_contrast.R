...
#' @export
lmertableloop_contrast <- function(d, m, iv, dv, con, p, s){
  ...
  for (i in dv) {
    ...
    cont <- as.data.frame(contrast(emmeans(lmer_results, iv), con))
    cont$p.value <- as.numeric(cont$p.value)
    cont <- t(cont)
    colnames(cont) <- cont[1,]
    cont <- as.data.frame(cont) %>% dplyr::filter(row.names(cont) %in% "p.value")

    line <- cbind(line, cont)

    DATA <- rbind(line, DATA)
  }

  DATA[, c(iv, colnames(cont))] <- lapply(DATA[, c(iv, colnames(cont))], as.numeric)

  DATA[, c(iv, colnames(cont))] <-  if(missing(p)){
    round(DATA[, c(iv, colnames(cont))], 3)
  }
  else if (p == 'full'){
    DATA[, c(iv, colnames(cont))]
  }
  else if (p == 'std1'){
    sapply(DATA[, c(iv, colnames(cont))], function(x) ifelse(x >= 0.01, round(x, digits = 2), ifelse(x < 0.001, "<0.001",
